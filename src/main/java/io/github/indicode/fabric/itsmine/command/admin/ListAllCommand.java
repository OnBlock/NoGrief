...
public class ListAllCommand {
    public static void register(LiteralArgumentBuilder<ServerCommandSource> command) {
        ...
        listall.executes(context -> listall(context.getSource(), 1, 10, BoolArgumentType.getBool(context, "offline")));
        page.then(entries);
        offline.then(page);
...
//                source.getMinecraftServer().getPlayerManager().
                source.getMinecraftServer().getPlayerManager().getPlayerList().forEach(serverPlayerEntity -> {
                    ClaimManager.INSTANCE.getPlayerClaims(serverPlayerEntity.getUuid()).forEach(claim1 -> {
                        claimList.add(new LiteralText(claim.name).formatted(Formatting.GOLD).append(new LiteralText(" ")).append(new LiteralText(String.valueOf(claim1.getEntities(source.getWorld()))).formatted(Formatting.YELLOW)));
                    });
                });
            }
        }
